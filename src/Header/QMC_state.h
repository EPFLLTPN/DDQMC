...
public:
	...
	gsl_rng * gsl_random;
	std::unordered_map<std::bitset<Bl>, Determinant>::iterator find_det;
	...
	std::unordered_map<std::bitset<Bl>, Determinant> determinant; //list of the flags of being an initiator
	...
	void Generate_Determinant(std::bitset<Bl> det, int nbr_re, int nbr_im, System_Model & sys);
	int SpawningAttempt(std::complex<double> & ampl, double pgen, gsl_rng * rand_gen, int parent, int child, int par_sign); //1-real, 0-imaginary
