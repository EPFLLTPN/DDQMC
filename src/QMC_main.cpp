...
//Global variables
int proc, myrank, master, D_Nbr, Init_Limit, energy_freq;
MPI_Request request;
int rest_iter = 0;
string path;
string restart_path = "";
...
int main(int argc, char *argv[]) {
	...
	if(myrank == master){
		...
		outPut_Sup = outPut;
		outPut_Sup.erase(outPut_Sup.size()-3);
		s.str("");
	...
	for(iter = rest_iter; iter < qmc_state->cycle; ++iter)	{
		...
		if(myrank == master)
		{
			...
			if(iter%energy_freq ==0)
				cout << iter <<" \t " << qmc_state->stats.N_tot <<" \t " <<  qmc_state->stats.N_w << "\t"
				...
				cout << iter <<" \t " << qmc_state->stats.N_w << qmc_state->stats.diag_walk_re +  qmc_state->stats.diag_walk_im << "\t " <<  qmc_state->pop_ctrl.shift << "\t"<< qmc_state->stats.det_nbr << "\t - \t - \t - \t"<< qmc_state->stats.diag_walk_re << "\t" << qmc_state->stats.diag_walk_im << endl;
	...
	else
	{
		...
		unordered_map<std::bitset<Bl>, Determinant>::iterator d = qmc_state->determinant.begin();
